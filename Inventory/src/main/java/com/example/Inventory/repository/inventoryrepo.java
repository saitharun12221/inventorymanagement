...
//     List<String> findAllDistinctCategories();
   @Query("select i.category, count(i), sum(i.stock) " + 
    ...
    @Query(value = "SELECT * FROM inventory WHERE category = ?1 AND subCategory = ?2 " +
                   "ORDER BY stock DESC, created_at DESC LIMIT ?3 OFFSET ?4", 
           nativeQuery = true)
    ...
    @Query("SELECT COUNT(i) FROM inventory i WHERE i.category = ?1 AND i.subCategory = ?2")
